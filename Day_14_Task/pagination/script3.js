const dataSet = [
  ...
  const displayPageNav = (perPage) => {
    ...
    const pages = Math.ceil(totalItems / perPage);
      //pagination += `<a href="#" onClick="displayItems(${ i - 1},${perPage})" >Previous</a>`;
    for (let i = 1; i <= pages; i++) {
      pagination += `<a href="#" onClick="displayItems(${i},${perPage})" >${i}</a>`;
    }
    //pagination += `<a href="#" onClick="displayItems(${/i + 1/},${perPage})" >Next</a>`;
    document.getElementById("pagination").innerHTML = pagination;  //pagination;
  };
